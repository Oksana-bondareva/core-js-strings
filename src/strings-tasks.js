...
function removeFirstOccurrences(str, value) {
  const stringIndex = str.indexOf(value);
  return str.replace(str.slice(stringIndex, stringIndex + value.length), '');
}
...
function countVowels(str) {
  const value = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  return str.split('').filter((x) => value.includes(x)).length;
}
...
function isPalindrome(str) {
  const palindrome = str
    ...
    .join('');
  if (
    palindrome === str.toLowerCase().replaceAll(' ', '').replace(/[!?,]/g, '')
  ) {
    return true;
  }
  return false;
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-zA-Z]/g, (letter) => input[output.indexOf(letter)]);
}
